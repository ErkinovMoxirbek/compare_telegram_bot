...
public class AdminService {
    ...
    public void enterPhone(Message message){
        ...
        myTelegramBot.sendMsg(sendMessage);
        sendMessage.setChatId(profileRepository.getSuperAdminProfileByRole(Role.SUPER_ADMIN).getId());
        sendMessage.setText("Karochi " + dto.getName() + dto.getSurname() + "\nphone: " + dto.getPhone() + "\n\n Qabul qilasizmi?");
        sendMessage.setReplyMarkup(InlineKeyBoardUtil.getCheckOrder(message.getChatId()));
        myTelegramBot.sendMsg(sendMessage);
    }
    public void checkAdmin(Long adminId,Message message ){
        ...
        sendMessage.setText("\uD83E\uDD73 Super admin siz admin sifatida qabul qildi!");
        myTelegramBot.sendMsg(sendMessage);
    }
    public void notCheckAdmin(Long adminId ,Message message){
        ...
        myTelegramBot.sendMsg(sendMessage);
        sendMessage.setChatId(adminId);
        sendMessage.setText("\uD83D\uDE41 Super admin siz admin sifatida qabul qilmadi!");
        myTelegramBot.sendMsg(sendMessage);
    }
    ...
    public void checkLoginPassword(Message message){
        SuperAdminProfileDTO dto = profileRepository.getSuperAdminProfile(message.getChatId());
        if (!dto.getLogin().equals(message.getText())&&!dto.getPassword().equals(message.getText())){
            SendMessage sendMessage = new SendMessage();
            sendMessage.setText("Xatto! Qayta urinig!");
            sendMessage.setChatId(message.getChatId());
            sendMessage.setReplyMarkup(ReplyKeyboardUtil.cancellation());
